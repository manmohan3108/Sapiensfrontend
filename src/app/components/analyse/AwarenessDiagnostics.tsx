...
import { getEnvironment } from '../../core/config/apiConfig';
...
export function AwarenessDiagnostics({ sapienId }: { sapienId: string }) {
  ...
  const staffLoginUrl = `${new URL(getEnvironment().apiBaseUrl).origin}/admin/login/?next=${encodeURIComponent(`/api/sapien/${sapienId}/awareness/beats?limit=20`)}`;
  ...
  const loadStaffBeats = useCallback(async () => {
    ...
    catch (caught) { const apiError = caught as ApiError; setBeatsError(apiError.status === 403 ? 'Staff session was not accepted. Sign in to Django administration with a staff account, then retry.' : apiError.message || 'Could not load awareness beat diagnostics.'); }
  ...
  return <div className="space-y-4">{error && <div className="flex items-center gap-2 rounded-xl border border-red-400/20 bg-red-400/[.07] px-4 py-3 text-xs text-red-200/75"><AlertCircle className="h-4 w-4"/><span className="flex-1">{error}</span><button onClick={() => void refresh()} className="rounded-lg border border-red-300/20 px-3 py-1.5">Retry</button></div>}{!beatData && <div className="flex flex-wrap items-center gap-3 rounded-xl border border-amber-400/20 bg-amber-400/[.07] px-4 py-3 text-xs text-amber-100/70"><ShieldAlert className="h-4 w-4"/><span className="min-w-56 flex-1">Beat diagnostics require an authenticated Django staff session. Cookies are sent only after you confirm sign-in.</span><a href={staffLoginUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-amber-300/20 px-3 py-2 text-amber-100 hover:bg-amber-300/10">Open staff sign-in</a><button onClick={() => void loadStaffBeats()} disabled={beatsLoading} className="rounded-lg bg-amber-300/15 px-3 py-2 text-amber-50 disabled:opacity-50">{beatsLoading ? 'Checking session…' : staffSessionReady ? 'Retry staff session' : 'I have signed in'}</button></div>}{beatsError && <div className="flex items-center gap-2 rounded-xl border border-red-400/20 bg-red-400/[.07] px-4 py-3 text-xs text-red-100/70"><AlertCircle className="h-4 w-4"/><span className="flex-1">{beatsError}</span></div>}
