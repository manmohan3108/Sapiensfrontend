...
import type { AwarenessBeat, AwarenessBeatEvent, AwarenessHistoryItem, AwarenessResponse } from '../../types/sapiensTypes';
...
export function AwarenessDiagnostics({ sapienId }: { sapienId: string }) {
  ...
  return <div className="space-y-4">{error && <div className="flex items-center gap-2 rounded-xl border border-red-400/20 bg-red-400/[.07] px-4 py-3 text-xs text-red-200/75"><AlertCircle className="h-4 w-4"/><span className="flex-1">{error}</span><button onClick={() => void refresh()} className="rounded-lg border border-red-300/20 px-3 py-1.5">Retry</button></div>}
    ...
    {currentBeat ? <BeatDetails beat={currentBeat} latest/> : <div className={`${panel} p-5 text-center`}><ShieldAlert className="mx-auto h-6 w-6 text-white/18"/><p className="mt-3 text-xs text-white/35">This backend returned the legacy Awareness shape.</p><p className="mt-1 text-[10px] text-white/22">Committed thought history remains available below.</p></div>}
