...
function InquiryList({ sapienId, kind }: { sapienId: string; kind: InquiryKind }) {
  ...
  const [simulation, setSimulation] = useState(Boolean(initialRun));
  const [revision, setRevision] = useState(0);
  ...
  const valid = Number.isInteger(filters.limit) && filters.limit >= 1 && filters.limit <= 200
    ...
    && (!simulation || Boolean(filters.simulation_run_id));
  ...
  return <div className="space-y-4">
    ...
    <section className={`${panel} p-4`}><div className="flex flex-wrap items-end gap-3">
      <label className="text-xs text-white/50">Run scope<select className={input} value={simulation ? 'simulation' : 'normal'} onChange={event => { const enabled = event.target.value === 'simulation'; setSimulation(enabled); change({ simulation_run_id: '' }); }}><option value="normal">Normal</option><option value="simulation">Specific simulation run</option></select></label>
      {simulation && <label className="min-w-64 flex-1 text-xs text-white/50">Simulation run ID<input className={input} maxLength={128} value={filters.simulation_run_id} onChange={event => change({ simulation_run_id: event.target.value.trim() })} placeholder="Exact simulation_run_id" /></label>}
      ...
      <button type="button" className={button} onClick={() => { setSimulation(false); change(defaults); }}>Reset</button>
