...
function LiveSimulatedTime({ value, endAt, speed, advancing }: { value: string; endAt: string; speed: number; advancing: boolean }) { const anchor = useRef({ value, endAt, speed, advancing, receivedAt: Date.now() }); const current = anchor.current; if (current.value !== value || current.endAt !== endAt || current.speed !== speed || current.advancing !== advancing) anchor.current = { value, endAt, speed, advancing, receivedAt: Date.now() }; const [, tick] = useState(0); useEffect(() => { if (!advancing) return; let frame = 0, lastPaint = 0; const paint = (now: number) => { if (now - lastPaint >= 250) { lastPaint = now; tick(count => count + 1); } frame = window.requestAnimationFrame(paint); }; frame = window.requestAnimationFrame(paint); return () => window.cancelAnimationFrame(frame); }, [advancing]); const snapshot = anchor.current; const base = new Date(snapshot.value).getTime(); const projected = snapshot.advancing ? base + (Date.now() - snapshot.receivedAt) * snapshot.speed : base; return <>{simulatedStamp(new Date(Math.min(projected, new Date(snapshot.endAt).getTime())))}</>; }
...
const hasLiveRuntime = (run: SimulationRun) => !run.archived && run.runtime_available !== false;
...
export function SimulationLabPage() {
  ...
  const setCurrentSapiens = useSapiensStore(state => state.setCurrentSapiens);
  const [returning, setReturning] = useState(false);
  const [params, setParams] = useSearchParams();
  const shortcutId = params.get('sapienId');
  const shortcutName = params.get('sapienName');
  const requestedRunId = params.get('runId') || '';
  const requestedView = params.get('view');
  const mainView = requestedView === 'participant' || requestedView === 'results' || requestedView === 'debug' ? requestedView : 'world';
  ...
  const chooseView = (view: 'world' | 'participant' | 'results' | 'debug') => { const next = new URLSearchParams(params); next.set('view', view); setParams(next, { replace: true }); };
  const chooseRun = (id: string) => { setSelectedId(id); const next = new URLSearchParams(params); next.set('runId', id); setParams(next, { replace: true }); };
  useEffect(() => { if (requestedView && !['world', 'participant', 'results', 'debug'].includes(requestedView)) chooseView('world'); }, [requestedView]);
  ...
  const returnPath = requestedReturn === '/workspace' || /^\/admin\/analyse(?:\/[a-z-]+)?(?:\?[^#]*)?$/.test(requestedReturn)
    ? requestedReturn : '/admin/analyse';
  async function returnToSapien() {
    if (!shortcutId) { navigate('/admin'); return; }
    setReturning(true);
    try {
      // Restore only the origin Sapien, never the run selected inside Simulation Lab.
      if (currentSapiens?.id !== shortcutId) {
        const available = await sapiensService.listSapiens();
        const origin = available.find(item => item.id === shortcutId);
        if (!origin) throw new Error('The original Sapiens is no longer available to this account.');
        setCurrentSapiens(origin);
      }
      navigate(returnPath);
    } catch (problem) { setError(message(problem)); setReturning(false); }
  }
  ...
  const visibleRuns = shortcutId ? runs.filter(item => String(item.sapien_id) === shortcutId) : runs;
  const refresh = useCallback(async (signal?: AbortSignal, clearError = true) => {
    ...
    const contextualRuns = shortcutId ? list.runs.filter(item => String(item.sapien_id) === shortcutId) : list.runs;
    setCatalog(cases); setRuns(list.runs); setHistoryLimit(list.history_limit); setSelectedId(id => requestedRunId && contextualRuns.some(item => item.run_id === requestedRunId) ? requestedRunId : id && contextualRuns.some(item => item.run_id === id) ? id : contextualRuns[0]?.run_id ?? '');
  ...
  useEffect(() => {
    if (!requestedRunId || runs.some(item => item.run_id === requestedRunId)) return;
    const controller = new AbortController();
    simulationService.get(requestedRunId, controller.signal).then(item => {
      if (shortcutId && String(item.sapien_id) !== shortcutId) { setError('The requested run does not belong to the contextual Sapiens.'); return; }
      setRuns(current => current.some(runItem => runItem.run_id === item.run_id) ? current : [item, ...current]);
      setSelectedId(item.run_id);
    }).catch(problem => { if (!controller.signal.aborted) setError(message(problem)); });
    return () => controller.abort();
  }, [requestedRunId, shortcutId, runs]);
  ...
  const drain = useCallback(async (id: string, kind: keyof typeof cursors.current, signal?: AbortSignal) => {
    ...
    for (let pageNumber = 0; ; pageNumber++) {
      ...
      if (pageNumber > 0 && pageNumber % 10 === 0) await new Promise<void>(resolve => window.setTimeout(resolve, 0));
    }
  }, []);
  useEffect(() => {
    ...
    const controller = new AbortController(); let timer = 0; let failed = 0; let inFlight = false;
    const poll = async () => {
      ...
      try {
        ...
        if (activityAvailable && mainView === 'participant' && next.participant_mode !== 'environment_only') {
          await Promise.all([drain(selectedId, 'messages', controller.signal), drain(selectedId, 'tools', controller.signal)]);
        }
        if (mainView === 'debug') {
          if (!guideLoaded.current) { setGuide(await simulationService.debug(selectedId, controller.signal)); guideLoaded.current = true; }
          if (activityAvailable) await Promise.all([drain(selectedId, 'diagnostics', controller.signal), drain(selectedId, 'evidence', controller.signal)]);
        }
        ...
        failed = 0;
        if (!(next.archived && terminal.has(next.status))) timer = window.setTimeout(poll, pollDelay(next.status));
      } catch (problem) {
    ...
    void poll(); return () => { controller.abort(); window.clearTimeout(timer); document.removeEventListener('visibilitychange', resume); };
  }, [selectedId, drain, mainView]);
  ...
  async function create() {
    ...
    try {
      ...
      pendingCreate.current = null; chooseRun(created.run_id); setRun(created);
      await refresh();
    } catch (problem) {
