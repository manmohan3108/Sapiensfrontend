...
export function SimulationLabPage() {
  ...
  const refresh = useCallback(async (signal?: AbortSignal, clearError = true) => {
    ...
    setCatalog(cases); setRuns(list.runs); setHistoryLimit(list.history_limit); setSelectedId(id => id && contextualRuns.some(item => item.run_id === id) ? id : contextualRuns[0]?.run_id ?? '');
