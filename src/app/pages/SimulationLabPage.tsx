...
const statusLabel = (status: SimulationStatus) => ({ starting: 'Preparing', scheduled: 'Ready', running: 'Running', paused: 'Paused', stopping: 'Stopping', completed: 'Completed', blocked: 'Blocked', stopped: 'Stopped', limit_reached: 'Limit reached', failed: 'Failed', created: 'Created' }[status]);
...
const helpFor = (label: string) => label.startsWith('Wall-time') ? 'The maximum real elapsed time this run may use. The standard default is six real hours, but the catalog value shown here is authoritative and may be lower. Preparation, scheduled waiting, and later pauses all count. Reaching it ends the run with limit_reached; it is not simulated duration.' : label.startsWith('Evidence') ? 'The maximum number of recorded events, including messages, tool activity, and diagnostics. It is not a token limit. Reaching it ends the run with limit_reached.' : label.includes('speed') ? 'How quickly simulated time advances relative to real time after execution starts. It does not make LLM or provider calls compute faster.' : null;
