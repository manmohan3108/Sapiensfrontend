...
const helpFor = (label: string) => label.startsWith('Wall-time budget') ? 'The maximum real elapsed time, in seconds, that this run may use. Paused time counts. Reaching it ends the run with limit_reached; it is not simulated duration.' : label.startsWith('Evidence limit') ? 'The maximum number of recorded events, including messages, tool activity, and diagnostics. It is not a token limit. Reaching it ends the run with limit_reached.' : label.includes('speed') ? 'How quickly simulated time advances relative to real time. It does not make LLM or provider calls compute faster.' : null;
...
export function SimulationLabPage() {
  ...
  useEffect(() => { pendingCreate.current = null; }, [caseIndex, participantId, participantName, selectedSapienId, speed, wall, records]);
  ...
  async function create() {
    ...
    const request: CaseCreateRequest = pendingCreate.current ?? { case: { case_id: selectedCase.case_id, version: selectedCase.version, start_at: selectedCase.suggested_start_at, participant_id: participantId, participant_name: participantName.trim() }, config: { run_id: `run-${newId()}`, ...(speedNumber !== undefined ? { speed: speedNumber } : {}), max_wall_seconds: wallNumber, max_records: recordNumber }, sapien_id: Number(selectedSapien.id) };
